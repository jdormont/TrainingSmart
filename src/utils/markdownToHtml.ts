...
export const convertMarkdownToHtml = (text: string): string => {
  ...
  return sanitized
    .replace(/<h1>/g, '<h1 class="text-2xl font-bold text-gray-900 mt-8 mb-4 break-words">')
    .replace(/<h2>/g, '<h2 class="text-xl font-bold text-gray-900 mt-8 mb-4 break-words">')
    .replace(/<h3>/g, '<h3 class="text-lg font-semibold text-gray-900 mt-6 mb-3 break-words">')
    .replace(/<p>/g, '<p class="mb-3 text-gray-700 leading-relaxed break-words">')
    .replace(/<ul>/g, '<ul class="mb-4 list-disc list-outside ml-4">')
    .replace(/<ol>/g, '<ol class="mb-4 list-decimal list-outside ml-4">')
    .replace(/<li>/g, '<li class="mb-1 text-gray-700 break-words">')
    .replace(/<a /g, '<a class="text-blue-600 hover:text-blue-800 underline break-all" target="_blank" rel="noopener noreferrer" ')
    .replace(/<blockquote>/g, '<blockquote class="border-l-4 border-gray-300 pl-4 italic text-gray-600 my-4 break-words">')
    .replace(/<pre>/g, '<div class="overflow-x-auto mb-4"><pre>')
