...
class HealthMetricsService {
  ...
  private calculateTrend(data: any[], type: string): 'improving' | 'stable' | 'declining' {
    ...
    switch (type) {
      case 'speed':
        recentAvg = data.slice(0, midpoint).reduce((sum, a) => sum + a.average_speed, 0) / midpoint;
        olderAvg = data.slice(midpoint).reduce((sum, a) => sum + a.average_speed, 0) / (data.length - midpoint);
        break;
      case 'distance':
        recentAvg = data.slice(0, midpoint).reduce((sum, a) => sum + a.distance, 0) / midpoint;
        olderAvg = data.slice(midpoint).reduce((sum, a) => sum + a.distance, 0) / (data.length - midpoint);
        break;
      case 'readiness':
        recentAvg = data.slice(0, midpoint).reduce((sum, r) => sum + r.score, 0) / midpoint;
        olderAvg = data.slice(midpoint).reduce((sum, r) => sum + r.score, 0) / (data.length - midpoint);
        break;
      case 'sleep':
        recentAvg = data.slice(0, midpoint).reduce((sum, s) => sum + s.efficiency, 0) / midpoint;
        olderAvg = data.slice(midpoint).reduce((sum, s) => sum + s.efficiency, 0) / (data.length - midpoint);
        break;
      default:
        return 'stable';
    }
