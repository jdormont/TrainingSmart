...
_Last cycle's Current Sprint item (test coverage for the persistent-memory feature) shipped as PR #44, merged 2026-06-23. No item is currently in flight._
...
### 1.1 Fix the `useMemorySessionSync` outgoing-session bug found in PR #44 — NEW
- **What:** PR #44's test author found and documented (via a passing test, not a fix) that `useMemorySessionSync` does not actually sync the outgoing session on session-switch, contradicting its own doc comment ("syncs on tab hidden, session switch, or unmount"). Root cause: `latestSessionRef.current` is reassigned to the new session during render, before the outgoing effect's cleanup (closed over the old `sessionId`) runs; the cleanup's own `session.id !== sessionId` guard then blocks the sync because the ref already points at the new session.
- **Why now:** This sits on the persistent-memory feature's core sync path — a user who switches chat sessions mid-conversation silently loses that session's memory update unless they also hit "tab hidden" or unmount first. It's a one-function fix with an existing test already proving the bug, found two cycles ago in PR #44 and not yet picked up; this is the natural point to act rather than let it go stale a third time.
- **Effort estimate:** S (half day — capture the outgoing session id in a ref *before* reassignment, or snapshot it in the effect's cleanup closure correctly)
- **Actual effort:** —
- **Agent prompt:** "In `src/hooks/useMemorySessionSync.ts`, fix the documented-but-broken outgoing-session sync on session switch (see PR #44 description and its test `does not sync the outgoing session when the active session is switched` in `useMemorySessionSync.test.ts`). The fix should snapshot the outgoing session id before `latestSessionRef.current` is reassigned to the new session, so the cleanup's sync call fires for the correct (old) session. Update the existing test to assert the corrected behavior (it currently documents the bug, not the fix). Run `npx vitest run` and confirm no regressions."

---

### 1.2 Eliminate `: any` in `stravaCacheService.ts` — OPEN _(carried, unchanged this cycle)_
...
### 1.3 SettingsPage Modularization — OPEN _(carried, unchanged this cycle)_
